...
class InitialCalculation:
    ...
    def Average(self, values):
        ...
        list1=[]
        for i in range (0,5):
            avg_transaction  = (avg_transaction +(values[i][0]*(i+1)))
            avg_tradedshare = (avg_tradedshare +(values[i][1]*(i+1)))
            avg_high = (avg_high+(values[i][2]*(i+1)))
            avg_low = (avg_low+(values[i][3]*(i+1)))
            avg_closed = (avg_closed +(values[i][4]*(i+1)))
        avg_transaction = avg_transaction/15
        avg_tradedshare = avg_tradedshare/15
        avg_high=avg_high/15
        avg_low=avg_low/15
        avg_closed=avg_closed/15
    ...
    def AverageNepse(self, values):
        ...
        for i in range (0,5):
            avg_banking  = (avg_banking +(values[i][0]*(i+1)))
            avg_devbank = (avg_devbank +(values[i][1]*(i+1)))
            avg_finance = (avg_finance+(values[i][2]*(i+1)))
            avg_hotel = (avg_hotel+(values[i][3]*(i+1)))
            avg_hydropower = (avg_hydropower +(values[i][4]*(i+1)))
            avg_insurance = (avg_insurance+(values[i][5]*(i+1)))
            avg_nepse = (avg_nepse+(values[i][6]*(i+1)))
            avg_others = (avg_others+(values[i][7]*(i+1)))
        avg_banking = avg_banking/15
        avg_devbank = avg_devbank/15
        avg_finance=avg_finance/15
        avg_hotel=avg_hotel/15
        avg_hydropower=avg_hydropower/15
        avg_insurance=avg_insurance/15
        avg_nepse=avg_nepse/15
        avg_others=avg_others/15        
...
def getHistoricalData(c_id):
    if(c_id == 0):
        ...
        conn = sqlite3.connect('chartereddata.sqlite3')
    elif (c_id ==4):
        ...
        conn = sqlite3.connect('nlicdata.sqlite3')
    elif (c_id ==8):
        conn = sqlite3.connect('alicldata.sqlite3')
    elif (c_id ==9):
        conn = sqlite3.connect('rhpcdata.sqlite3')
    elif (c_id ==10):
...
def analyzeId(c_id):
    ...
    if (c_id>0):
        ...
        averageError = 1      
        while (averageError>0.003 and itr<5000):
        #for i in range(1000):
            count1 =0
            resultBackProp =[]
            while (count1<295):
        ...
        nep_network = NeuralNetwork(inputNodes = 8, hiddenNodes = 16, outputNodes = 1) #company id =0 for nepse prediciton
        averageError = 1      
        #while (averageError>0.0008):
        while (averageError>0.003):
        #for i in range(1000):
            count1 =0
            resultBackProp =[]
